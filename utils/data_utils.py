...
#     # Affine transformation matrix for the computation
#     transformation_matrix = get_transformation_matrix(program_json, schedule_json, ordered_comp_list[0])
#     transformation_matrix = np.pad(
#                 transformation_matrix,
#                 [
#                     (0, MAX_DEPTH - transformation_matrix.shape[0]),
#                     (0, MAX_DEPTH - transformation_matrix.shape[1]),
#                 ],
#                 mode="constant",
#                 constant_values=0,
#             )
#     # Padd the transformation matrix
#     transformation_matrix = list(transformation_matrix.flatten())
#     output = output + transformation_matrix
...
#         assert(len(comp_schedule_dict["transformations_list"]) == 1 and comp_schedule_dict["transformations_list"][0][0] == 1)
#         i_loop = comp_schedule_dict["transformations_list"][0][1]
#         j_loop = comp_schedule_dict["transformations_list"][0][2]
...
#     output = output + interchange
    ...
    for iter_i, iterator_name in enumerate(comp_dict["iterators"]):
        ...
        if iterator_name == comp_schedule_dict["parallelized_dim"]:
                parallelized = 1
                parallelized_loop = iter_i
    output = output + [tiled]
#     output = output + [
#         tiled, 
#         tile_factor_indices[0] if len(tile_factor_indices)>0 else 0, 
#         tile_factor_indices[1] if len(tile_factor_indices)>0 else 0,
#         tile_factor_indices[2] if len(tile_factor_indices)>2 else 0,
#         tile_factors[0] if len(tile_factors)>0 else 0, 
#         tile_factors[1] if len(tile_factors)>0 else 0,
#         tile_factors[2] if len(tile_factors)>2 else 0
#     ]
